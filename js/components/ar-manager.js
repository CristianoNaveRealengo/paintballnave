...
AFRAME.registerComponent("ar-manager", {
			...
			navigator.xr
				...
				.then((supported) => {
					if (supported) {
						...
						console.warn(
							"⚠️ WebXR AR não suportado neste dispositivo"
						);
						this.fallbackToVR();
					}
				});
	...
	tryAnchorArena: function (plane) {
		...
		if (arenaEl) {
			...
			arenaEl.setAttribute(
				"scale",
				`${this.data.arenaScale} ${this.data.arenaScale} ${this.data.arenaScale}`
			);

			// Posicionar no plano
			arenaEl.setAttribute(
				"position",
				`${plane.position.x} ${plane.position.y + 0.01} ${
					plane.position.z
				}`
			);
	...
	autoPlaceArena: function () {
		...
		this.detectedPlanes.forEach((plane) => {
			...
			const distance = Math.sqrt(
				plane.position.x ** 2 + plane.position.z ** 2
			);
	...
	showArenaPlacedConfirmation: function () {
		...
		confirmEl.setAttribute(
			"background",
			"color: #000000; opacity: 0.8; padding: 0.2"
		);
		...
		this.el.sceneEl.emit("arena-ready", {
			position: this.arenaEntity.getAttribute("position"),
		});
	},
	...
	getARStatus: function () {
		return {
			...
			arenaPosition: this.arenaEntity
				? this.arenaEntity.getAttribute("position")
				: null,
		};
	},
});
