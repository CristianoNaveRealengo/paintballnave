...
class GameInitializer {
    ...
    async detectDevice() {
        ...
        if (navigator.xr) {
            try {
                ...
                this.gameState.isVRSupported = await navigator.xr.isSessionSupported('immersive-vr');
                
                // Verificar suporte AR
                this.gameState.isARSupported = await navigator.xr.isSessionSupported('immersive-ar');
                
                console.log(`📱 WebXR - VR: ${this.gameState.isVRSupported}, AR: ${this.gameState.isARSupported}`);
        ...
        if (userAgent.includes('quest') || userAgent.includes('oculus')) {
            this.gameState.deviceType = this.gameState.isARSupported ? 'ar' : 'vr';
        ...
        console.log(`🎯 Dispositivo detectado: ${this.gameState.deviceType}, Modo: ${this.gameState.currentMode}`);
    ...
    async loadSettings() {
        ...
        try {
            ...
            if (savedSettings) {
                this.gameState.settings = { ...defaultSettings, ...JSON.parse(savedSettings) };
            } else {
                this.gameState.settings = defaultSettings;
            }
        } catch (error) {
            console.warn('⚠️ Erro ao carregar configurações salvas, usando padrões:', error);
            this.gameState.settings = defaultSettings;
        }
    ...
    async setupScene() {
        ...
        if (this.gameState.currentMode === 'ar') {
            webxrConfig.requiredFeatures = ['local', 'hit-test', 'plane-detection'];
            webxrConfig.optionalFeatures = ['anchors', 'hand-tracking', 'light-estimation'];
    ...
    getComponentsForMode() {
        ...
        switch (this.gameState.currentMode) {
            ...
            case 'desktop':
                return [...baseComponents, 'mouse-controls', 'keyboard-controls'];
            default:
        ...
        switch (componentName) {
            ...
            case 'quest-hand-tracking':
                ...
                if (leftHand && !leftHand.hasAttribute('quest-hand-tracking')) {
                    leftHand.setAttribute('quest-hand-tracking', { hand: 'left' });
                }
                
                if (rightHand && !rightHand.hasAttribute('quest-hand-tracking')) {
    ...
    async setupAudio() {
        ...
        if (window.AudioManager) {
            ...
            if (audioSettings.spatialAudio && this.gameState.currentMode !== 'desktop') {
    ...
    async setupNetwork() {
        ...
        if (this.gameState.gameMode === 'multiplayer') {
            ...
            console.log('🔗 Modo multiplayer detectado - configuração de rede pendente');
        }
    ...
    setupPerformanceMonitoring() {
        ...
        const monitorPerformance = () => {
            ...
            if (currentTime - lastTime >= 1000) {
                ...
                if (fps < 30 && this.gameState.settings.graphics.quality !== 'low') {
                    console.warn('⚠️ FPS baixo detectado, reduzindo qualidade gráfica');
    ...
    saveGameState() {
        try {
            ...
            localStorage.setItem('paintball-vr-state', JSON.stringify(stateToSave));
        } catch (error) {
