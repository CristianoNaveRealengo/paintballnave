...
const gameState = {
    ...
    projectiles: [],
    healthPacks: [
        { id: 'health1', position: { x: 12, y: 1, z: 12 }, active: true, respawnTime: 0 },
        { id: 'health2', position: { x: -12, y: 1, z: -12 }, active: true, respawnTime: 0 }
    ],
    specialWeapon: {
...
function getAvailableSpawn() {
    const usedSpawns = Array.from(gameState.players.values()).map(p => p.spawnIndex);
...
function startGame() {
    if (gameState.players.size >= 2 && !gameState.gameActive) {
        ...
        console.log('🎮 Jogo iniciado com', gameState.players.size, 'jogadores');
        io.emit('gameStarted', { 
...
io.on('connection', (socket) => {
    ...
    socket.on('hit', (data) => {
        ...
        if (shooter && target && gameState.gameActive && shooter.id !== target.id) {
            // Aplicar dano
            target.health = Math.max(0, target.health - data.damage);
            
            // Adicionar pontos ao atirador
            shooter.score += data.damage;
            
            console.log(`💥 ${shooter.name} acertou ${target.name} (Dano: ${data.damage})`);
            ...
            if (target.health <= 0) {
                console.log(`💀 ${target.name} foi eliminado por ${shooter.name}`);
    ...
    socket.on('collectHealthPack', (data) => {
        const player = gameState.players.get(socket.id);
        const healthPack = gameState.healthPacks.find(pack => pack.id === data.packId);
        ...
        if (player && healthPack && healthPack.active && player.health < 100) {
            ...
            console.log(`❤️ ${player.name} coletou health pack (+${healAmount} HP)`);
    ...
    socket.on('collectSpecialWeapon', () => {
        ...
        if (player && gameState.specialWeapon.active) {
            ...
            console.log(`🔫 ${player.name} coletou arma especial: ${gameState.specialWeapon.type}`);
